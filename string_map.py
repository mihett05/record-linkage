...
class StringMap:
    ...
    def shrink(self, centroids: np.ndarray | None = None, clusters: np.ndarray | None = None) -> None:
        ...
        self.strings = np.array([self.strings[clusters == label][0] for label in np.unique(clusters)])
    ...
    def add_new_strings(self, strings: list[str]) -> None:
        ...
        self.coords: np.ndarray = np.vstack([self.coords, np.zeros((len(strings), self.dimensions), dtype=float)])
