...
from preprocess.dataset1 import parse_dataset1_row, Dataset1Row

fake_data = [
    Dataset1Row(uid=UUID('e13cf60d-b63a-4d93-8014-1468aa87aae1'), name='БУЛАН БАМАН ЧОРИЕВИЧ', email='bamanbulan2712671', address='Златоуст,Высотный,6/5,537,005088', sex='m', birthdate='2972-11-24', phone='85589856357'),
    Dataset1Row(uid=UUID('b0a915e4-b257-4328-8014-144f2a92fd25'), name='МУЗЫКАНТОВА ТАИСИЯ ЧИНГ', email='taisijamuzykantova5522165', address=',Запорожский,23,4,417936', sex='f', birthdate='1991-12-11', phone='85202803460'),
    Dataset1Row(uid=UUID('aad6c6a4-832c-4d27-8014-11de72ff874a'), name='БАЙДАВЛЕТОВ МАХКАМДЖОН ВАЛИЕВИЧ', email='mahkamdzhonbajdavletov5152570', address=',Смирнова,92,3,044111', sex='m', birthdate='2001-05-04', phone='88991724737'),
    Dataset1Row(uid=UUID('e13cf60d-4d93-4d93-8014-1468aa87aae1'), name='БУЛАН БАМАН ЧООРИЕВИЧ', email='bamanbuuulan2712671', address='Златоуст,Высотный,6/б,537,005088', sex='m', birthdate='2972-11-24', phone='85589856357'),
    Dataset1Row(uid=UUID('b0a915e4-4d93-4328-8014-144f2a92fd25'), name='МУЗЫКАНТОВА ТАИСИЯ ТАИСИЯ ЧИНГ', email='taisijamuzykantova5522165', address=',Запорожский,23,4,417936', sex='f', birthdate='1991-12-11', phone='85202803460'),
    Dataset1Row(uid=UUID('aad6c6a4-b63a-4d27-8014-11de72ff874a'), name='БАЙДАВЛЕТОВ МАХКАМДЖОН ВАЛИЕВИЧ', email='mahkamdzhonbajdavletov5152570', address=',Смирнова,92,3,044111', sex='m', birthdate='2001-05-04', phone='88991724737'),
]
...
def calculate_weight(row):
    total_weight = 0
    for attr in ['name_comparison', 'email_comparison', 'address_comparison', 'sex_comparison', 'birthdate_comparison', 'phone_comparison']:
        m = m_probs[attr]
        u = u_probs[attr]
        if row[attr] >= 0.8:
            weight = log(m / u)
